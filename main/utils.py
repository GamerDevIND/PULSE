...
class Logger:
    ...
    @classmethod
    async def log_async(cls, message: str, level, append = True, stdout = True, save_to_file = True):
        text = cls._format_message(message, level)
...
def convert_funcs(*funcs):
    ...
    for func in funcs:
        ...
        for arg_name, param in sig.parameters.items():
            ...
            if "description" not in properties[arg_name]: properties[arg_name]["description"] = f"The {arg_name} parameter."

            if param.default == inspect._empty: required.append(arg_name)
        data = {
            "type": "function",
            "function": {
                "name": name,
