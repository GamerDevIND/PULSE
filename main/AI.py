    ...
    async def route_query(self, query: str, manual: bool):
        if not manual:
            ...
            if selected_role in self.models:
                ...
                await log(f"⚠️ Router selected unknown role or failed to parse ('{selected_role}'). Using default '{self.default_model}'.", "warn")
                return query, self.default_model
        else:
            if query.startswith("!chat"):
                ...
                return query, self.default_model
    async def generate(self, query, stream: None | bool = None, manual_routing = False, seperate_thinking = False):
        ...
        async for res in model.generate(query, self.context['conversations'], stream):
            if res == ERROR_TOKEN:
                save = False
                break
