...
class Summariser:
    def __init__(self, model:RemoteModel | LocalModel | None,  summary_max_tokens, summary_keep_tokens_after, min_recent_turns) -> None:
    ...
    async def maybe_summarise_context(self, context, prev_summary : None | str = None, prev_facts : None | list[str] = None, summary_system_prompt = SUMMARIZER_PROMPT, auto_warm_up = False):
        ...
        if (len(total_words) * 1.3) >= self.summary_max_tokens:  
            ...
            used = 0
            for t in context[:-self.min_recent_msgs]:
            ...
            if entry:
                ...
                facts = entry["facts"]  
                context = context[-self.min_recent_msgs:]   
